...
export default function RecentViews() {
    ...
    const fetchViewedCars = () => {
        ...
        }).then(resp => resp.json()).then(response => setState({
            ...state,
    ...
    const renderSingleCarItem = (carItem: carData[]) => {
        return (<FlatListView showsHorizontalScrollIndicator={false} data={carItem}
                              renderItem={({item, index}: any) => <ViewedCarList carData={item} index={index}/>}
                              ...
                              key={'_'} contentContainerStyle={{margin: 5}}/>);
    }
    return (
        <SafeAreaView style={{
            ...sharedStyles.container
        }}>
            {state.viewedCars.length > 0 ? renderSingleCarItem(state.viewedCars) : <Animated.View style={{
                padding: 10,
                ...recentViewsStyles.noRecentViews, justifyContent: "center", alignItems: "center",
            }}><Text style={{
